...
metadata_comment = generate.style_transfer(original_video_frames,
        eng_config=config,
        current_source_frame_image_weight=5.0, # effects how closely the output video will be to the source video. Values of 0.1-3.0 are reasonable.
...
if RIFE_interpolation:
        ...
        RIFE_output_filename = os.path.splitext(generated_video_no_audio)[0] + f'_{RIFE_interpolation_factor}X_{int(video_exraction_framerate*RIFE_interpolation_factor)}fps.mp4\"'
...
if copy_audio:
        ...
        video_tools.copy_video_audio(input_video_path, generated_video_no_audio, final_video_filename)
