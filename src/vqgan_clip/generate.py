...
def video_frames(eng_config=VQGAN_CLIP_Config(),
    ...
    z_deque = collections.deque([eng._z.clone(),eng._z.clone(),eng._z.clone(),eng._z.clone(),eng._z.clone(),eng._z.clone(),eng._z.clone(),eng._z.clone(),eng._z.clone(),eng._z.clone()])
    current_prompt_number = 0
    video_frame_num = 1
    # generate images
    try:
        for iteration_num in tqdm(range(1,iterations+1),unit='iteration',desc='video'):
            ...
            if save_every and iteration_num % save_every == 0:
                ...
                if smoothed:
                    # Remove the rightmost _z, and add the latest _z to the left.
                    z_deque.pop()
                    z_deque.appendleft(eng._z.clone())
                    # elementwise average of tensors
                    mean_z = torch.mean(torch.stack(list(z_deque)), dim=0)
