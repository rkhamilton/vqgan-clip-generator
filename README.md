...
See [Real-ESRGAN.md](Real-ESRGAN.md) for installation instructions for use with this package.
```sh
conda activate vqgan
...
In addition to the setup commands above, run the following commands to set up RIFE. Please note that RIFE does not offer an installable python package, unlike the packages above. You will have to clone their repository to the working directory you plan to use for your VQGAN+CLIP projects.
```sh
conda activate vqgan
...
```
Then, [download the RIFE trained model v3.8](https://drive.google.com/file/d/1O5KfS3KzZCY3imeCr2LCsntLhutKuAqj/view?usp=sharing) to the ./arXiv2020-RIFE/train_log/ folder.
...
|generate.style_transfer()|Apply VQGAN_CLIP to each frame of a video. This is an enhancement of the standard style transfer algorithm that has improvements to fluidity of the resulting video. The result is a folder of images that can be combined using (e.g.) ffmpeg.|
...
## Parameters for all generate functions (image, video_frames, style_transfer)
...
These parameters are passed to the functions of vqgan_clip.generate: image(), video_frames(), style_transfer().
...
|[style_transfer_exploration.py](examples/style_transfer_exploration.py)|Explore the output of generate.style_transfer() for combinations of parameter values. Useful to dial in the settings for your style transfer.|

### Style Transfer
The method generate.style_transfer() will apply VQGAN+CLIP prompts to an existing video by extracting frames of video from the original and using them as inputs to create a frame of output video. The resulting frames may be combined into a video, and the original audio is optionally copied to the new file. As an example, here is a video of my face restyled with the prompt "portrait on deviantart" and an init_weight of 1.0 ([full code to generate this video](examples/style_transfer.py)).  
...
* From the starting point of the previous video frame, the training process evolves the image toward the next source frame of video, thereby tracking the original source video frame-by-frame. Increasing current_source_frame_image_weight causes the output video to follow the source more closely.
